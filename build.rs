...
fn main() {
    ...
    let mut xdr_rs = String::from(
        r#"
//auto generated by build.rs, don't modify this
...
"#,
    ...
    for entry in
        std::fs::read_dir(XDR_X_PATH).expect(&format!("directory '{}' doesn't existed", XDR_X_PATH))
    {
        match entry {
            Ok(entry) => {
                if !entry
                    ...
                    .is_file()
                {
                    ...
                    xdr_rs.push_str(&format!(
                        r#"include!(concat!(env!("OUT_DIR"), "\\{}_xdr.rs"));"#,
                        entry
                            .file_name()
                            .to_str()
                            .unwrap()
